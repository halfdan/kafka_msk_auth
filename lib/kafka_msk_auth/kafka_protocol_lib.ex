defmodule KafkaMskAuth.KafkaProtocolLib do
  @moduledoc """
  ...
  @callback make(atom(), non_neg_integer(), list() | map()) ::
              {:kpro_req, reference(), atom(), non_neg_integer(), false, binary() | list() | map()}
  def make(api, version, fields), do: :kpro_req_lib.make(api, version, fields)

  @callback send_and_recv(
              {:kpro_req, reference(), atom(), non_neg_integer(), boolean(), binary() | list() | map()},
              port(),
              atom(),
              binary(),
              :infinity | non_neg_integer()
            ) :: list({atom() | integer(), any()}) | map()
  def send_and_recv(req, sock, mod, client_id, timeout), do: :kpro_lib.send_and_recv(req, sock, mod, client_id, timeout)
end
